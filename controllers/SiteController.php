...
class SiteController extends Controller
{
    ...
    public function actionSeeder()
    {
        ...
        for ($i=0; $i < 50; $i++) {
            ...
            $usuario = User::find()->orderBy('rand()')->one();
            
            $categoria = new Categoria();
            $categoria->categoria   = $faker->unique()->word;
            $categoria->slug        = $faker->unique()->slug;
            $categoria->imagen      = $faker->unique()->slug . '.png';
            $categoria->descripcion = $faker->text(200);
            $categoria->created_by  = $usuario->id;
            $categoria->created_at  = new Expression('NOW()');
            $categoria->updated_by  = $usuario->id;
            $categoria->updated_at  = new Expression('NOW()');
            $categoria->insert();
            echo $categoria->categoria . " insertada correctamente<br>";
        }
    }
}
